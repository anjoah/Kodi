...
bool CAMLCodec::OpenDecoder(CDVDStreamInfo &hints)
{
  ...
  switch(am_private->video_format)
  {
    ...
    case VFORMAT_H264_4K2K:
      if ((aml_get_device_type() == AML_DEVICE_TYPE_M8) || (aml_get_device_type() == AML_DEVICE_TYPE_M8M2)) {
        ...
        CLog::Log(LOGDEBUG, "CAMLCodec::OpenDecoder codec init failed, 4K supported only on Amlogic S802 and S812.");
        return false;
      }
      break;
    case VFORMAT_HEVC:
  ...
  if (aml_mode_to_resolution(mode, &res))
    if ((StringUtils::StartsWith(mode, "4k2k")) || (aml_get_device_type() <= AML_DEVICE_TYPE_M3))
    {
